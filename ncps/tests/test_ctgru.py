"""Tests for CT-GRU cell implementation."""

import pytest
import keras
import numpy as np
from ncps.layers.ctgru import CTGRUCell


def test_initialization():
    """Test CT-GRU initialization."""
...
def test_build():
    ...
    cell.build((None, 64))
    assert cell.built
    assert cell.kernel.shape == (96, 96)  # (input_dim + units, 3 * units)
    
    # Test with backbone
    cell = CTGRUCell(32, backbone_units=128, backbone_layers=1)
    cell.build((None, 64))
    assert cell.built
    assert cell.kernel.shape == (128, 96)  # (backbone_units, 3 * units)
...
def test_gates():
    ...
    # Values should be bounded by activation functions
...
def test_activations():
    ...
    # Test different activation combinations
    configs = [
        ("tanh", "sigmoid"),
        ("relu", "hard_sigmoid"),
        ("selu", "sigmoid")
    ...
    for act, rec_act in configs:
        cell = CTGRUCell(32, activation=act, recurrent_activation=rec_act)
        rnn = keras.layers.RNN(cell)
        output = rnn(inputs)
