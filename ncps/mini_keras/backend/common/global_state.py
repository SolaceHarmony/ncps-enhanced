...
@keras_mini_export(["ncps.keras_mini.utils.clear_session", "ncps.keras_mini.backend.clear_session"])
def clear_session(free_memory=True):
    """Resets all state generated by ncps.mini_keras.
    ...
    for _ in range(100):
      ...
      model = ncps.mini_keras.Sequential([
          ncps.mini_keras.layers.Dense(10) for _ in range(10)])

    for _ in range(100):
      # With `clear_session()` called at the beginning,
      # Keras starts with a blank state at each iteration
      # and memory consumption is constant over time.
      ncps.mini_keras.backend.clear_session()
      model = ncps.mini_keras.Sequential([
          ncps.mini_keras.layers.Dense(10) for _ in range(10)])
    ```
    ...
    >>> new_layer = ncps.mini_keras.layers.Dense(10)
