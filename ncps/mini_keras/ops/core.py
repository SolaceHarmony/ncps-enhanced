...
@keras_mini_export("ncps.mini_keras.ops.map")
def map(f, xs):
    ...
    >>> f = lambda x: x**2
    >>> xs = ncps.mini_keras.ops.arange(10)
    >>> ys = ncps.mini_keras.ops.map(f, xs)
    >>> ys
...
@keras_mini_export("ncps.mini_keras.ops.scan")
def scan(f, init, xs=None, length=None, reverse=False, unroll=1):
    ...
    >>> init = ncps.mini_keras.ops.array(0)
    >>> xs = ncps.mini_keras.ops.array([1, 2, 3, 4, 5])
    >>> carry, result = ncps.mini_keras.ops.scan(sum_fn, init, xs)
    >>> carry
    15
    >>> result
    [1, 3, 6, 10, 15]
...
@keras_mini_export("ncps.mini_keras.ops.associative_scan")
def associative_scan(f, elems, reverse=False, axis=0):
    ...
    >>> xs = ncps.mini_keras.ops.arange(5)
    >>> ys = ncps.mini_keras.ops.associative_scan(sum_fn, xs, axis=0)
    >>> ys
    [0, 1, 3, 6, 10]

    >>> sum_fn = lambda x, y: [x[0] + y[0], x[1] + y[1], x[2] + y[2]]
    >>> xs = [ncps.mini_keras.ops.array([[1, 2]]) for _ in range(3)]
    >>> ys = ncps.mini_keras.ops.associative_scan(sum_fn, xs, axis=0)
    >>> ys
    [[1, 3], [1, 3], [1, 3]]
    """
...
@keras_mini_export("ncps.mini_keras.ops.scatter")
def scatter(indices, values, shape):
    ...
    ```python
    zeros = ncps.mini_keras.ops.zeros(shape)
    output = ncps.mini_keras.ops.scatter_update(zeros, indices, values)
    ```
    ...
    >>> indices = [[0, 1], [1, 1]]
    >>> values = np.array([1., 1.])
    >>> ncps.mini_keras.ops.scatter(indices, values, shape=(2, 2))
    array([[0., 1.],
           [0., 1.]])
    """
...
@keras_mini_export("ncps.mini_keras.ops.scatter_update")
def scatter_update(inputs, indices, updates):
    ...
    inputs = ncps.mini_keras.ops.scatter_update(inputs, indices, updates)
    ```
...
@keras_mini_export("ncps.mini_keras.ops.slice")
def slice(inputs, start_indices, shape):
    ...
    shape = np.array([2, 2])
    inputs = ncps.mini_keras.ops.slice(inputs, start_indices, shape)
    ```
...
@keras_mini_export("ncps.mini_keras.ops.slice_update")
def slice_update(inputs, start_indices, updates):
    ...
    start_indices = [3, 3]
    updates = np.ones((2, 2))
    inputs = ncps.mini_keras.ops.slice_update(inputs, start_indices, updates)
    ```
...
@keras_mini_export("ncps.mini_keras.ops.switch")
def switch(index, branches, *operands):
    ...
    >>> x = ncps.mini_keras.ops.array(2.0)
    >>> y = ncps.mini_keras.ops.array(0.5)
    >>> branches = [add_fn, subtract_fn]
    >>> ncps.mini_keras.ops.switch(0, branches, x, y)
    2.5

    >>> ncps.mini_keras.ops.switch(1, branches, x, y)
    1.5
    """
...
@keras_mini_export("ncps.mini_keras.ops.while_loop")
def while_loop(
    cond,
    body,
    loop_vars,
    ...
    >>> cond = lambda i: i < 10
    >>> body = lambda i: i + 1
    >>> ncps.mini_keras.ops.while_loop(cond, body, i)
    10

    >>> x, y = 0, 1
    >>> cond = lambda x, y: x < 10
    >>> body = lambda x, y: (x + 1, y + 1)
    >>> ncps.mini_keras.ops.while_loop(cond, body, (x, y))
    10, 11
    """
    return backend.core.while_loop(
        cond,
...
@keras_mini_export("ncps.mini_keras.ops.stop_gradient")
def stop_gradient(variable):
    ...
    ...     dtype="float32"
    ... )
    >>> var = ncps.mini_keras.ops.stop_gradient(var)
    """
...
@keras_mini_export("ncps.mini_keras.ops.fori_loop")
def fori_loop(lower, upper, body_fun, init_val):
    ...
    >>> init_val = 0
    >>> ncps.mini_keras.ops.fori_loop(lower, upper, body_fun, init_val)
    45
    """
...
@keras_mini_export("ncps.mini_keras.ops.unstack")
def unstack(x, num=None, axis=0):
    ...
    >>> x = ncps.mini_keras.ops.array([[1, 2], [3, 4]])
    >>> ncps.mini_keras.ops.unstack(x, axis=0)
    [array([1, 2]), array([3, 4])]
    """
...
@keras_mini_export("ncps.mini_keras.ops.shape")
def shape(x):
    ...
    >>> x = ncps.mini_keras.ops.zeros((8, 12))
    >>> ncps.mini_keras.ops.shape(x)
    (8, 12)
    """
    if any_symbolic_tensors((x,)):
        return x.shape
...
@keras_mini_export("ncps.mini_keras.ops.dtype")
def dtype(x):
...
@keras_mini_export("ncps.mini_keras.ops.cast")
def cast(x, dtype):
    ...
    >>> x = ncps.mini_keras.ops.arange(4)
    >>> x = ncps.mini_keras.ops.cast(x, dtype="float16")
    """
...
@keras_mini_export("ncps.mini_keras.ops.saturate_cast")
...
@keras_mini_export("ncps.mini_keras.ops.convert_to_tensor")
def convert_to_tensor(x, dtype=None, sparse=None, ragged=None):
    ...
    >>> x = np.array([1, 2, 3])
    >>> y = ncps.mini_keras.ops.convert_to_tensor(x)
    """
    if any_symbolic_tensors((x,)):
...
@keras_mini_export("ncps.mini_keras.ops.convert_to_numpy")
...
@keras_mini_export("ncps.mini_keras.ops.cond")
...
@keras_mini_export("ncps.mini_keras.ops.vectorized_map")
...
@keras_mini_export("ncps.mini_keras.ops.is_tensor")
def is_tensor(x):
...
@keras_mini_export("ncps.mini_keras.ops.custom_gradient")
