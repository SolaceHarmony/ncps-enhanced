...
from ncps.mini_keras.ops.operation_utils import compute_conv_output_shape
...
@keras_mini_export("ncps.mini_keras.ops.image.rgb_to_grayscale")
...
@keras_mini_export("ncps.mini_keras.ops.image.rgb_to_hsv")
...
@keras_mini_export("ncps.mini_keras.ops.image.hsv_to_rgb")
...
@keras_mini_export("ncps.mini_keras.ops.image.resize")
def resize(
    images,
    size,
    interpolation="bilinear",
    ...
    >>> y = ncps.mini_keras.ops.image.resize(x, (2, 2))
    >>> y.shape
    (2, 2, 2, 3)
...
@keras_mini_export("ncps.mini_keras.ops.image.affine_transform")
def affine_transform(
    ...
    ...     ]
    ... )
    >>> y = ncps.mini_keras.ops.image.affine_transform(x, transform)
    >>> y.shape
    (2, 64, 80, 3)
...
@keras_mini_export("ncps.mini_keras.ops.image.extract_patches")
def extract_patches(
    images,
    size,
    strides=None,
    ...
    >>> patches = ncps.mini_keras.ops.image.extract_patches(image, (5, 5))
    >>> patches.shape
    (2, 4, 4, 75)
    >>> image = np.random.random((20, 20, 3)).astype("float32") # 1 RGB image
    >>> patches = ncps.mini_keras.ops.image.extract_patches(image, (3, 3), (1, 1))
    >>> patches.shape
    (18, 18, 27)
    """
    if any_symbolic_tensors((images,)):
...
@keras_mini_export("ncps.mini_keras.ops.image.map_coordinates")
def map_coordinates(
...
@keras_mini_export("ncps.mini_keras.ops.image.pad_images")
def pad_images(
    images,
    top_padding=None,
    left_padding=None,
    ...
    >>> padded_images = ncps.mini_keras.ops.image.pad_images(
    ...
    >>> padded_batch = ncps.mini_keras.ops.image.pad_images(
...
@keras_mini_export("ncps.mini_keras.ops.image.crop_images")
def crop_images(
    images,
    top_cropping=None,
    left_cropping=None,
