...
class DynamicBackend:
    ...
    def __getattr__(self, name):
        if self._backend == "tensorflow":
            module = importlib.import_module("ncps.mini_keras.backend.tensorflow")
            return getattr(module, name)
        if self._backend == "jax":
            module = importlib.import_module("ncps.mini_keras.backend.jax")
            return getattr(module, name)
        if self._backend == "torch":
            module = importlib.import_module("ncps.mini_keras.backend.torch")
            return getattr(module, name)
        ...
        if self._backend == "openvino":
            module = importlib.import_module("ncps.mini_keras.backend.openvino")
            return getattr(module, name)


@keras_mini_export("ncps.mini_keras.config.set_backend")
