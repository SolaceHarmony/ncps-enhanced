"""Continuous-time RNN (CTRNN) cell implementation."""
...
class CTRNNCell(BackboneLayerCell):
    """A Continuous-time RNN cell.
    
    This implements a basic continuous-time recurrent neural network
    with time-constant based updates.
    ...
    def call(
        ...
        h_tilde = ops.matmul(x, self.kernel) + self.bias
        h_tilde = self.activation(h_tilde)
        
        # Compute time constants
        tau = ops.exp(self.tau_kernel(x))
        
        # Update state
        h_new = h_prev + t * (-h_prev + h_tilde) / tau
