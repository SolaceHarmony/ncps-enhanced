...
class LTCRNN(nn.Module):
    """An RNN layer using LTC (Liquid Time-Constant) cells.
    ...
    For each element along the sequence length axis, this layer applies the LTC
    ...
    def __init__(
        ...
        for layer in range(num_layers):
            ...
            # Forward cell
            self.forward_layers.append(LTCRNNCell(
            ...
            if bidirectional:
                self.backward_layers.append(LTCRNNCell(
                    input_size=layer_input_size,
                    hidden_size=hidden_size,
                    bias=bias,
                    activation=activation,
    ...
    def __call__(
        ...
        """Process input sequence through the LTC layers.
        ...
        for layer in range(self.num_layers):
            ...
            for t in range(seq_len):
                ...
                output, state = forward_cell(current_input[:, t], state, time_delta=dt)
            ...
            if self.bidirectional:
                ...
                for t in range(seq_len - 1, -1, -1):
                    dt = time_delta[:, t] if isinstance(time_delta, mx.array) else time_delta
                    output, state = backward_cell(current_input[:, t], state, time_delta=dt)
